...
### 4. **Element Model (M<sub>el</sub>) with CP Optimizer**
   - Run using the command:  
     `python3 cpo_element.py -f [problem_filepath]`

### 5. **Table Model (M<sub>tb</sub>) with CP Optimizer**
   - Run using the command:  
     `python3 cpo_table.py -f [problem_filepath]`

### 5. **Element Model (M<sub>el</sub>) with OR-Tools CP-SAT**
   - Run using the command:  
     `python3 ort_element.py -f [problem_filepath]`

### 6. **Table Model (M<sub>tb</sub>) with OR-Tools CP-SAT**
   - Run using the command:  
     `python3 ort_table.py -f [problem_filepath]`

### 7. **Element Model (M<sub>el</sub>) with OscaR**
   - Run using the command:  
     `java -Xms1g -Xmx30g -jar ./OscaR/oscar-element.jar [problem_filepath] domwdeg`

### 8. **Table Model (M<sub>tb</sub>) with OscaR**
   - Run using the command:  
